...
FancyObject_p class_method_File_open__mode__with(FancyObject_p self, list<FancyObject_p> args, Scope *scope)
{
  ...
  assert(IS_STRING(arg1->native_value()));
  assert(IS_STRING(arg2->native_value()));
  assert(IS_BLOCK(arg3->native_value()));
  
  string filename = dynamic_cast<String_p>(arg1->native_value())->value();
  string mode = dynamic_cast<String_p>(arg2->native_value())->value();
  Block_p block = dynamic_cast<Block_p>(arg3->native_value());
  ...
  FancyObject_p file_object = FileClass->create_instance(file);
  
  block->call(self, list<FancyObject_p>(1, file_object), scope);
...
{
  File_p file = dynamic_cast<File_p>(self->native_value());
  if(file) {
